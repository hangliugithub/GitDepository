    ...
    <head>
        ...
        <script language="javascript" type="text/javascript">
        	$(function(){
        		...
        		console.log(deleted);
        		if(deleted){
        			$("#operate_result_info").slideDown("slow");
        		}
        	});
            ...
            function deleteFee(btn) {
            	//alert("nihao");
            	//获取btn所在tr中第一个td的值
            	var costId = $(btn).parent().siblings().html();
            	//alert(costId);
                var r = window.confirm("确定要删除id为"+costId+"资费吗？");
                if(r==true){
                	//将costId绑定到隐藏input
                	$("#choose").val(costId);
                	alert($("#choose").val());
                	$("#form1").attr("action","delCost.do");
                }
            }
            
            //修改
            function modifyCost(btn){
            	//获取btn所在tr中第一个td的值
            	var costId = $(btn).parent().siblings().html();
            	...
            	$("#choose").val(costId);
            	//alert($("#choose").val());
            	$("#form1").attr("action","toModifyCost.do");
            }
        </script>        
    </head>
    <body>
            ...
            <form action="" method="post" id="form1">
                ...
                <div id="data">            
                    <table id="datalist">
                        ...
                        <tr>
                            <td>${c.costId }</td>
                            <td><a href="detialCost.do?id=${c.costId }">${c.name }</a></td>
                ...
                <!-- 该隐藏域用来发送所要操作的那条资费的id -->
                <input type="hidden" id="choose" name="costId" ></input>
            </form>
        </div>
        <!--主要区域结束-->
        <div id="footer">
